...
export function CameraScanner({ onScan, onClose }) {
  ...
  const firedRef = useRef(false); // sync ref — kein useState, kein Race Condition
  ...
  useEffect(() => {
    ...
    reader
      .decodeFromConstraints(
        ...
        (result) => {
          ...
          if (text.length >= 10) {
            firedRef.current = true; // synchron sperren — kein weiterer Durchlauf
            // Kamera sofort stoppen, bevor Callbacks weiterlaufen
    ...
    return () => {
      firedRef.current = true; // auch beim Unmount sperren
      try { BrowserMultiFormatReader.releaseAllStreams(); } catch (_) {}
    };
      ...
      <div className="flex-1 relative overflow-hidden">
        {error ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center px-8 text-center gap-4">
            ...
            <button onClick={onClose} className="px-5 py-2 bg-amber-500 rounded-lg text-white text-sm font-medium">
              Schliessen
            </button>
          </div>
        ) : (
          <>
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="relative w-72 h-40">
                ...
                <Corner pos="top-0 right-0" flipX />
                <Corner pos="bottom-0 left-0" flipY />
                <Corner pos="bottom-0 right-0" flipX flipY />
...
function Corner({ pos, flipX, flipY }) {
  return (
    <div
