...
export function BookGrid({ books, shelves = [], onSelect, viewMode = 'grid' }) {
  ...
  return (
    <div className={`grid ${GRID_COLS[viewMode] ?? GRID_COLS.grid} gap-3 p-4`}>
      {books.map((book) => (
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center flex-1 py-24 px-6 text-center">
      <svg className="w-16 h-16 text-stone-300 mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <h2 className="text-xl font-semibold text-stone-400 mb-2">Noch keine Bücher</h2>
