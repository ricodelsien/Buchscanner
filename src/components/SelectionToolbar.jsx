    ...
    <div className="fixed bottom-0 left-0 right-0 z-40 pb-safe">
      ...
      {shelfOpen && (
        <div className="mx-4 mb-2 theme-surface rounded-2xl shadow-2xl border border-stone-100 dark:border-stone-800 overflow-hidden max-h-52 overflow-y-auto">
          ...
          <button onClick={() => setShelfOpen(false)} className="w-full px-4 py-2.5 text-sm text-stone-400 hover:text-stone-600 text-center border-t border-stone-100 dark:border-stone-800">
            Abbrechen
          </button>
        </div>
      )}
      ...
      {/* Main toolbar */}
      <div className="theme-surface border-t border-stone-200 dark:border-stone-800 shadow-2xl px-4 py-3 flex items-center gap-3">
        {/* Count + select all */}
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="text-sm font-medium text-stone-700 dark:text-stone-300 truncate">
            {count} {count === 1 ? 'Buch' : 'Bücher'} ausgewählt
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {shelves.length > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-stone-600 dark:text-stone-300 border border-stone-200 dark:border-stone-700 hover:border-stone-400 dark:hover:border-stone-500 transition-colors"
            >
